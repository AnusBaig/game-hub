...
const VideoPlayer = ({ media, autoplay = false, showControls = true, onDownload }: Props) => {
  ...
  useEffect(() => {
    ...
    const handleError = (e: Event) => {
      ...
      setHasError(true);
      setErrorMessage('Failed to load video. The video format may not be supported.');
      setIsLoading(false);
    };
    ...
    if (autoplay && !hasError) {
      video.play().catch((error) => {
        ...
        // Autoplay failed, but don't show error - this is common due to browser policies
      });
    }
          ...
          {isLoading ? (
            <Center bg="gray.100">
              <VStack spacing={3}>
                ...
                <Text color="gray.600" fontSize="sm">Loading YouTube video...</Text>
              </VStack>
            </Center>
          ) : hasError ? (
            <Box 
